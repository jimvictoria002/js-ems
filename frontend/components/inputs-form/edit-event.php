<div class="w-full px-3">
    <div class="w-full bg-white rounded-lg p-6 mt-2 mb-64">
        ...
        <?php elseif (!($access == 'staff' || $access == 'admin') && $event['status'] == 'approved') : ?>
            <div class="flex justify-start my-3 flex-col">
                <div class="flex flex-col">
                    <p class="text-lg font-semibold my-2 text-green-700  ml-1">Status: <?= ucfirst($event['status']) ?></p>
                    <!-- <h1
                        class="p-1 tex-sm md:text-lg rounded-lg mt-1 font-semibold inline-block  <?= $event['status'] == 'approved' ? 'bg-green-700' : 'bg-orange-700' ?>  text-white capitalize px-7">
                        <?= $event['status'] ?>
                    </h1> -->
                ...
                <div class="flex flex-col">
                    <p class="text-sm font-semibold text-orange-700  ml-1">Updating this event will put it back in pending status for admin approval.</p>
                    <!-- <h1
                        class="p-1 tex-sm md:text-lg rounded-lg mt-1 font-semibold inline-block  <?= $event['status'] == 'approved' ? 'bg-green-700' : 'bg-orange-700' ?>  text-white capitalize px-7">
                        <?= $event['status'] ?>
                    </h1> -->
        ...
        <form action="../backend/update/update_event.php" class="mt-4" method="POST" class="mt-2" id="create-event" enctype="multipart/form-data">
            ...
            <div class="event-attributes flex items-start gap-y-6 gap-[3%] flex-wrap">
                ...
                <div class="w-full text-sm md:text-base sm:w-[47%] lg:w-[31%] flex flex-col">
                    ...
                    <input type="text" placeholder="Enter title" class="form-input p-1 border active:border-green-950 rounded-sm w-full" value="<?= $event['title'] ?>" name="title">
                </div>

                <!-- Description -->
                <div class="w-full text-sm md:text-base sm:w-[47%] lg:w-[31%] flex flex-col">
                    <label for="" class="font-semibold">Description</label>
                    <input type="text" placeholder="Enter description" class=" form-input p-1 border active:border-green-950 rounded-sm w-full" value="<?= $event['description'] ?>" name="description">
                </div>
                ...
                <div class="w-full text-sm md:text-base sm:w-[47%] lg:w-[31%] flex flex-col">
                    ...
                    ?>
                    <select name="venue" id="select-venue" class="form-input p-1 border active:border-green-950 rounded-sm w-full ">
                        <option value="">--</option>
                    ...
                    <div>
                        <input type="text" name="input-venue" id="input-venue" class="form-input hidden p-1 border active:border-green-950 rounded-sm w-full" placeholder="Enter venue">
                    </div>
                ...
                <div class="w-full text-sm md:text-base sm:w-[47%] lg:w-[31%] flex flex-col p-3 border  rounded-md">
                    <div class="flex justify-between">
                        <label for="event_img" class="font-semibold">Uploaded image<span class="text-red-700">*</span></label>
                        <p class="text-green-700 text-xs mt-1">Maximum of 10MB<span class="text-red-700">*</span></p>
                    </div>

                    <div id="image-preview" class="mt-2 ">
                        <img src="<?= '../uploads/event_img/' . $event['event_img'] ?>" alt="event_img" class="max-w-full" id="event-img-preview">
                    </div>

                    <!-- <p id="view-note" class="text-sm text-green-700 font-bold mt-2  cursor-pointer">View image</p> -->

                    <input type="file" id="event_img" name="event_img" accept="image/*" class="form-input p-1 border mt-3 active:border-green-950 rounded-sm w-full text-sm cursor-pointer" onchange="previewImage(event)">
                ...
                <div class="w-full text-sm md:text-base sm:w-[47%] lg:w-[31%] flex flex-col">
                    ...
                    <input type="datetime-local" class="form-input p-1 border active:border-green-950 rounded-sm w-full" value="<?= $event['start_datetime'] ?>" name="start_datetime">
                </div>

                <!-- Start time -->
                <div class="w-full text-sm md:text-base  sm:w-[47%] lg:w-[31%] flex flex-col">
                    <label for="" class="font-semibold">End date/time<span class="text-red-700">*</span></label>
                    <input type="datetime-local" class="form-input p-1 border active:border-green-950 rounded-sm w-full" value="<?= $event['end_datetime'] ?>" name="end_datetime">
                </div>
            ...
            <div class="w-full flex gap-y-10 flex-col-reverse md:flex-row  justify-end my-4 pr-5 mb-5 md:gap-10">
                ...
                <?php if ($access == 'admin' || $access == 'staff') : ?>
                    <?php if ($event['status'] == 'pending') : ?>
                        ...
                        <button type="button" onclick="if($('#create-event').valid())if(confirm('Do you really want to approve this event?'))$('#approve-<?= $event_id ?>').submit();" class="px-8 py-2  self-start md:text-base text-sm bg-green-500 hover:bg-green-400  cursor-pointer transition-default text-white font-semibold rounded-xl">Approve
                        </button>
                    <?php else : ?>
                        <button type="button" onclick="if($('#create-event').valid())if(confirm('Do you really want to Unapprove this event?'))$('#approve-<?= $event_id ?>').submit();" class="px-8 py-2  self-start md:text-base text-sm bg-orange-500 hover:bg-orange-400  cursor-pointer transition-default text-white font-semibold rounded-xl">Remove
                            approval</button>
                    <?php endif; ?>
                ...
                <div class="flex self-end gap-10">
                    <button disabled class="px-8 py-2 self-end md:text-base text-sm bg-green-800 cursor-not-allowed opacity-70 transition-default text-white font-semibold rounded-xl" id="upt-btn">Update</button>
                    <button type="button" onclick="if(confirm('Do you really want to delete this event?'))$('#delete-event').submit();" class="px-8 py-2 self-end md:text-base text-sm bg-red-700 hover:bg-red-600 cursor-pointer  transition-default text-white font-semibold rounded-xl">Delete</button>
                </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#create-event').validate({
                    ...
                    messages: {},
                    submitHandler(form) {
                        if (!("<?= $access ?>" == 'staff' || "<?= $access ?>" == 'admin') && '<?= $event['status'] ?>' == 'approved') {
                            if (confirm("If you update this event, the event will be on pending again")) {
                                form.submit();
                            }
                        } else {
                            form.submit();
